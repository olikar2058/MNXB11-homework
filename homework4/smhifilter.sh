...
# Select only measurements with temperature between 10 and 20 degrees Celsius
FILTER_FILTERFILENAME_TEMP_RANGE="temprange_$FILTER_FILTEREDFILENAME"
log "Filtering on only temperatures between 10 and 20 degrees Celsius, writing to $FILTER_FILTERFILENAME_TEMP_RANGE"
awk '$3 >= 10 && $3 <= 20 {print $0}' $CLEANER_BAREDATAFILENAME > $FILTER_FILTERFILENAME_TEMP_RANGE
